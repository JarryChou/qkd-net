...
public class QNLKeyReader {
    ...
    public void read(String src, String dest, String blockId,
        ...
        try {
            ...
            //keys is an empty vector that we want to read in the keys and store inside
            //the file at sb.toString() should already contain the keys written out, so we just want to read them in
            //and store them in the vector keys
        } catch(Exception e) {
            e.printStackTrace();
        }
    }
